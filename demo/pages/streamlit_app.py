...
def clean_numeric(df, cols):
    """
    Cleans specified numeric columns in a DataFrame.
    Converts to string, removes '$' and ',', and coerces to numeric.
    """
    df_clean = df.copy()
    for col in cols:
...
# Popularity Analysis View
    ...
    try:
        ...
        with col1:
            ...
            fig_bf.update_layout(yaxis={'categoryorder':'total ascending'}, 
                                 yaxis_title=None, xaxis_title="Total Meals Served")
        ...
        with col2:
            ...
            fig_ln.update_layout(yaxis={'categoryorder':'total ascending'}, 
                                 yaxis_title=None, xaxis_title="Total Meals Served")
        ...
        with col3:
            # Aggregate top 15 leftover breakfast items
            top_bf_left = df_b.groupby('name')['left_over_total'].sum().reset_index()
            top_bf_left = top_bf_left.sort_values(by='left_over_total', ascending=False).head(15)
            
            # Create breakfast chart
            fig_bf_left = px.bar(
                top_bf_left, 
                x='left_over_total', 
                y='name', 
                orientation='h', 
                title="Top 15 Leftover Breakfast Items",
                color_discrete_sequence=['red'] # <-- Make red
            )
            fig_bf_left.update_layout(
                yaxis={'categoryorder':'total ascending'},
                yaxis_title=None, 
                xaxis_title="Total Units Left Over"
            )
            st.plotly_chart(fig_bf_left, use_container_width=True)

        with col4:
            # Aggregate top 15 leftover lunch items
            top_ln_left = df_l.groupby('name')['left_over_total'].sum().reset_index()
            top_ln_left = top_ln_left.sort_values(by='left_over_total', ascending=False).head(15)
            ...
            fig_ln_left = px.bar(
                ...
                y='name', 
                orientation='h', 
                title="Top 15 Leftover Lunch Items",
                color_discrete_sequence=['red'] # <-- Make red
            )
            fig_ln_left.update_layout(
                yaxis={'categoryorder':'total ascending'},
                yaxis_title=None, 
                xaxis_title="Total Units Left Over"
            )
        ...
        st.error(f"An error occurred while loading popularity data: {e}")
        st.exception(e)
...
# Recommendation View
    ...
    if st.sidebar.button(f"Generate Report for {selected_school.title()}"):
